...
> **데이터 소스 변경 이력 (구현 중 확정):** 키움 REST → KRX → **네이버**.
> - 키움 REST는 **등록 IP에서만** 호출 가능 → 동적 IP인 GitHub Actions/Cloudflare와 부적합.
> - KRX(data.krx.co.kr)는 일자 지정(백필) 장점이 있으나 **데이터센터 IP를 봇 차단**(`LOGOUT`) → 클라우드 자동화 불가.
> - **최종: 네이버 모바일 API(m.stock.naver.com)** — 무인증·무IP, 클라우드에서 동작 확인. 단 **당일 데이터만** 제공하여 과거 백필은 불가(원래의 전진 수집 결정 유지). 전 종목을 받아 거래대금 상위 100을 직접 정렬하며 **ETF/ETN은 제외**(개별 종목 대상). 단위: 거래대금=백만원, 시총=억원 → 원으로 환산.
> 아래 본문의 KRX/키움 관련 서술은 이 결정으로 대체된다. 그 외(스키마·Function·프론트·병합·강조 로직)는 그대로 유효하다.

## 요약

기존 주식 전광판 화면 **아래**에, 대한민국 개장일마다 장 마감 후 **거래대금 상위 100종목**을 엑셀처럼 정리하는 "재료정리" 보드를 추가한다. 순위·전일순위·종목코드·종목명·현재가·등락률·시가총액·거래대금·시총대비 거래대금 비율은 KRX 공개 데이터(data.krx.co.kr)에서 자동 수집하고, 신규/기존·테마·재료·재료지속성·재료연속여부·재무·수급은 사용자가 화면에서 직접 입력해 저장한다. 데이터는 단일 공용·날짜별 파일(`data/jaelyo/YYYY-MM-DD.json`)로 GitHub 저장소에 보관하며, 기존 가격 스냅샷과 동일하게 **GitHub Action 일 1회 수집 + Pages Function read/write** 구조를 따른다. 날짜는 드롭다운으로 선택하고, 등락률·시총·거래대금·시총대비비율 임계값을 넘는 셀은 배경색으로 강조한다.
...
핵심 제약:
- 거래대금 **상위 랭킹**과 **시가총액**은 기존 앱이 쓰는 네이버/Yahoo 종목별 시세로는 한 번에 얻기 어렵다 → KRX 전종목 시세 한 번 호출로 해결한다.
- 키움 REST는 등록 IP에서만 호출 가능해 동적 IP인 GitHub Actions·Cloudflare와 맞지 않아 **KRX 공개 데이터(무인증·무IP)** 로 전환했다.
- KRX는 일자 지정이 되므로 과거(2026-05-01~) 백필이 가능하다. 기본은 배포 후 매일 전진 수집, 백필은 `JAELYO_BACKFILL_FROM`으로 선택 실행.
...
- **KTD1 — 데이터 소스: KRX 공개 데이터(data.krx.co.kr).** 전종목 시세(MDC `getJsonData`, bld `MDCSTAT01501`)를 일자 지정으로 1회 POST 호출해 종목코드·종목명·종가·등락률·거래대금·시가총액을 모두 얻는다. **인증·앱키·IP 등록 불필요.** (키움 REST는 등록 IP에서만 호출 가능해 동적 IP인 GitHub Actions/Cloudflare와 부적합 → KRX로 전환)
  - *실행 시점 확인 필요*: KRX `bld`·필드명(`ISU_SRT_CD`·`ACC_TRDVAL`·`MKTCAP` 등)·필요 헤더(`Referer`)는 실응답 1회로 검증. 매핑이 `parseAllStocks` 한 곳에 격리됨.
- **KTD2 — 시가총액 보강 불필요.** KRX 전종목 응답에 `MKTCAP`(시가총액)이 포함되어 별도 종목별 조회·스로틀 루프가 없다. 거래대금 내림차순 상위 100을 코드에서 정렬·선정(`rankByTradingValue`)한다.
- **KTD3 — 단위는 원(KRW) 그대로.** KRX는 거래대금·시총을 원 단위로 제공하므로 환산이 필요 없다. 임계값: 시총 ≤ 2e12, 거래대금 ≥ 4e11, 시총대비비율 = 거래대금/시총×100 > 20. (4천억/2조 = 20% 와 일치 — `ref/매일_재료정리.png`의 23.60·22.45 등 값과 부합)
...
flowchart TD
    ...
    subgraph Core["functions/api/_jaelyo-core.js (순수)"]
        P["전종목 파서 · 거래대금 상위선정 · 비율계산 · 전일순위 diff · 정규화"]
    end
    K[("KRX data.krx.co.kr")]
...
### U1. KRX 수집 코어 (`functions/api/_jaelyo-core.js`)

**목표** — 네트워크와 분리 가능한 순수 로직 + 얇은 fetch 래퍼로 KRX 수집 파이프라인을 구성한다. 기존 `_quotes-core.js`/`_search-core.js`의 "순수 파서 + 얇은 fetch" 구조를 그대로 따른다.
...
- 순수 함수(네트워크 없음, 테스트 대상):
  - `parseAllStocks(json)` → KRX 전종목 응답 → `[{code, name, price, changePct, tradingValue(원), marketCap(원)}]`
  - `rankByTradingValue(rows, limit)` → 거래대금 내림차순 상위 N + `rank`(1..N)·`tvToMcapPct` 부여
  ...
  - `toKrxDate(date)` → `YYYY-MM-DD` → `YYYYMMDD`
  - `normalizeBoard({date, rows})` → 최종 스키마(빈 `manual` 기본값 포함)
- 얇은 네트워크 래퍼(테스트 미대상, 모킹 경계):
  - `fetchAllStocks(date, opts)` → KRX `getJsonData`(bld `MDCSTAT01501`) POST → `parseAllStocks`. 인증 없음. 휴장/미래일자는 빈 목록.
...
**Test scenarios**
- `parseAllStocks`: 정상 응답 → 코드/등락률/거래대금·시총(원, 콤마 제거) 매핑 정확. code 없는 행 제외. 빈 `OutBlock_1` → `[]`, 비배열 → throw.
- `rankByTradingValue`: 거래대금 내림차순 상위 N, `rank` 1..N, `tvToMcapPct` 계산, 시총 null이면 비율 null. 거래대금 null 행 제외.
...
- `normalizeBoard`: 알 수 없는 필드 제거, 7개 manual 키 항상 존재, `source` 기본 `krx`.
- `toKrxDate`: `2026-05-07` → `20260507`.

---

### U2. 일별 수집 스크립트 + GitHub Action (`scripts/jaelyo-snapshot.mjs`, 워크플로)

**목표** — 장 마감 후 1회 KRX에서 거래대금 상위 100을 수집·정규화·병합해 `data/jaelyo/YYYY-MM-DD.json`으로 커밋한다. 과거 백필(선택)도 지원. 기존 `scripts/snapshot.mjs` + `.github/workflows/snapshot.yml` 구조를 복제·확장한다.
...
- `.dev.vars.example` (수정 — 키움 키 제거, KRX 무인증·백필 안내)

**Approach**
- `collectDay(dir, date)`: `fetchAllStocks(date)` → `rankByTradingValue(상위 100)` → 직전 개장일 파일 찾기(`data/jaelyo/`에서 date 미만 최신) → `prevRank`/`manual` 병합 → 파일 쓰기. 빈 결과(휴장)면 미작성·계속.
- `main()`: 기본은 KST 오늘 1건. `JAELYO_BACKFILL_FROM=YYYY-MM-DD` 설정 시 그 날짜~오늘 달력일을 오름차순 수집(`enumerateDates`), 휴장일은 자동 스킵, KRX 예의상 간격을 둔다.
- 휴장/빈 응답 가드: KRX가 비거래일에 빈 목록을 주므로 `rankByTradingValue` 결과가 비면 파일 미작성.
- 출력 경로 `data/jaelyo/${date}.json`. `today`는 Action 환경(UTC)에서 KST 보정(`kstDateString`).
- 워크플로: `snapshot.yml`과 동일 골격. cron은 **장 마감 후 1회**(평일 06:40 UTC ≈ 15:40 KST). **시크릿 불필요(KRX 공개)**, 변경 시 `git commit -m "...[skip ci]"`.
...
- 백필 날짜 열거: `enumerateDates(from, to)` 오름차순·월 경계 포함, 잘못된 범위 → `[]`.
- 휴장 가드: 빈 결과 → 파일 미작성.
- `Test expectation`: KRX HTTP 호출(`fetchAllStocks`)은 모킹 경계로 두고 직접 테스트하지 않음(네트워크는 실행시 검증).
...
- **신규 시크릿 없음**: KRX 공개 데이터라 수집에 인증·시크릿이 필요 없다. 수집 Action 실패해도 기존 전광판/가격 스냅샷에는 영향 없음.
...
- **KRX 응답 계약(bld·필드명·헤더) 불확실성** — 완화: `parseAllStocks` 한 곳에 매핑을 격리하고, 실제 필드명(`ISU_SRT_CD`·`ACC_TRDVAL`·`MKTCAP` 등)·필요 헤더(`Referer`)는 구현 첫 단계에서 1회 실호출로 확정한 뒤 픽스처화.
...
- **KRX 비공식 엔드포인트** — MDC `getJsonData`는 공개 데이터이나 비공식 인터페이스라 변경·차단 가능성 존재. 백필 시에는 호출 간격을 두어 과부하를 피한다.
...
- **2026-05-01~ 과거 백필**: KRX 일자 지정으로 가능해졌다. `JAELYO_BACKFILL_FROM=2026-05-01`로 1회 실행하면 채워진다(즉시 실행은 선택, 기본 동작은 전진 수집).
...
- 수집 스크립트: `node scripts/jaelyo-snapshot.mjs` 1회 실행해 오늘 파일 생성·필드 검증(KRX 실응답으로 필드명 확정).
...
- KRX `getJsonData`의 정확한 `bld`·응답 필드명(`ISU_SRT_CD`·`ACC_TRDVAL`·`MKTCAP` 등)·필요 헤더(`Referer`/`User-Agent`)는 실응답 1회로 확정.
- KRX 세션 쿠키 필요 여부: 쿠키 없이 POST하면 JSON 대신 `LOGOUT` 등이 올 수 있음. 그 경우 로더 페이지 GET → `set-cookie` → `Cookie` 헤더 2단계로 보강(`fetchAllStocks` 주석에 위치 표시). 첫 실호출로 확정.
- 휴장 판별에 쓸 응답 일자 필드.
