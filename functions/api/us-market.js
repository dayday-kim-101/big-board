// GET /api/us-market?email=              → { dates, latest, report }
// GET /api/us-market?email=&date=YYYY-MM-DD → 해당일 미국증시 리포트 + 사용자 한줄메모
// PUT /api/us-market?email=&date=YYYY-MM-DD body:{ memo }
import { emailKey, normalizeEmail, readJson, writeJson, listDir } from './_github.js';
import { applyMemo, sanitizeReport } from './_us-market-core.js';

const REPORT_DIR = 'data/us-market';
const NOTE_DIR = 'data/us-market-notes';
...
export async function onRequestGet({ request, env }) {
  ...
  try {
    ...
    return err(`미국증시 리포트 로드 실패: ${e.message}`, 502);
  }
}
...
export async function onRequestPut({ request, env }) {
  ...
  try {
    ...
    await writeJson(env, notePath(hash), saved, `chore: 미국증시 ${date} 한줄메모 갱신`);
    ...
    return err(`미국증시 메모 저장 실패: ${e.message}`, 502);
  }
}
